...
class UnitycatalogLineageSource(Source):
    ...
    def _resolve_lineage_side(
        self, full_name: str | None, path: str | None
    ...
    def _parse_column_pairs(raw_column_pairs: str | None) -> list[tuple[str, str]]:
        ...
        try:
            ...
            logger.debug(
                "Skipping unreadable column pairs %r: %s", raw_column_pairs, exc
            )
            return []
        return sorted(
        ...
        try:
            with self.engine.connect() as conn:
                rows = conn.execution_options(
                    stream_results=True, max_row_buffer=LINEAGE_ROW_BUFFER_SIZE
                ).execute(
                    text(
                        unity_catalog_native_lineage_query(
                            query_log_duration, include_query_history
                        )
                    )
                )
                ...
                for row in rows:
                    ...
                    source_tables, source_path = self._resolve_lineage_side(
                        row.source_table_full_name, row.source_path
                    )
            ...
            logger.info(
                "Native lineage: emitted %s edges over %s targets", emitted, targets
            )
        except Exception as exc:
    ...
    def _process_target_lineage(
        ...
        for target_table in sorted(target_tables):
            ...
            yield from self._process_table_lineage(
                table, target_table, upstream_columns, upstream_sql
            )
    ...
    def _get_table_entity(self, databricks_table_fqn: str) -> Table | None:
        ...
        entity_fqn = fqn._build(  # pylint: disable=protected-access
            self.config.serviceName, catalog_name, schema_name, table_name
        )
    ...
    def _process_external_location_lineage(
        ...
        try:
            ...
            if location_entity and location_entity[0]:
                ...
                if not table:
                    logger.debug(
                        "Unable to find external table: %s", databricks_table_fqn
                    )
                    return
            ...
            logger.debug(
                f"Error processing external location lineage for {databricks_table_fqn}: {exc}"
            )
            logger.debug(traceback.format_exc())
        ...
        for storage_path in sorted(upstream_paths):
            try:
                ...
                for candidate in container_path_candidates(storage_path):
                    location_entity = self.metadata.es_search_container_by_path(
                        full_path=candidate, fields="dataModel"
                    )
                    if location_entity and location_entity[0]:
                        break
    ...
    def _process_table_lineage(
        ...
        for source_table_full_name in sorted(upstream_columns):
            ...
            try:
                ...
                if not from_entity:
                    logger.debug(
                        f"Unable to find upstream entity: {source_table_full_name} -> {databricks_table_fqn}"
                    )
                    continue
                ...
                logger.debug(
                    f"Error processing lineage {source_table_full_name} -> {databricks_table_fqn}: {exc}"
                )
                logger.debug(traceback.format_exc())
