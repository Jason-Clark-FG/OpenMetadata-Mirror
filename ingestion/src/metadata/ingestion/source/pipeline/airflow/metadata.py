...
class AirflowSource(PipelineServiceSource):
    ...
    def _set_observability_context(
        self,
        pipeline_entity: Optional[Pipeline] = None,  # noqa: UP045
        table_fqns: Optional[List[str]] = None,  # noqa: UP006, UP045
        latest_dag_run: Optional[DagRun] = None,  # noqa: UP045
    ) -> None:
        """
    ...
    def get_table_pipeline_observability(
        ...
        try:
            ...
            if not (pipeline_entity and table_fqns and latest_dag_run):
                logger.debug(f"No observability data to emit for dag {pipeline_details.dag_id}")
                return
            ...
            try:
                ...
                logger.warning(
                    f"Failed to build observability for dag run {latest_dag_run.run_id} "
                    f"of {pipeline_details.dag_id}: {exc}"
                )
                logger.debug(traceback.format_exc())
                return

            table_pipeline_map: Dict[str, List[PipelineObservability]] = {  # noqa: UP006
            ...
            logger.info(
                ...
                f"{len(table_pipeline_map)} tables"
            )
