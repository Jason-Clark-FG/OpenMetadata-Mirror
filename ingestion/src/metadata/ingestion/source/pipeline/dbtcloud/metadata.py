    ...
    def get_table_pipeline_observability(
        ...
        try:
            ...
            if not (pipeline_entity and table_fqns and latest_run):
                logger.debug(f"No observability data to emit for job {pipeline_details.id}")
                return
            ...
            logger.debug(f"Emitting observability for {len(table_pipeline_map)} tables of job {pipeline_details.id}")
