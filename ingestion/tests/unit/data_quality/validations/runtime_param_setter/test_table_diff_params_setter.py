...
from metadata.utils.matchers import AnInstanceOf, HasProperties, InAnyOrder
...
def test_setter_gets_default_key_columns(
    ...
    assert setter.get_parameters(test_case) == AnInstanceOf(
        TableDiffRuntimeParameters
    ) & HasProperties(
        keyColumns=["id"],
        extraColumns=InAnyOrder(["name", "table_id"]),
        table1=AnInstanceOf(TableParameter)
        & HasProperties(
            key_columns=["id"],
        ),
        table2=AnInstanceOf(TableParameter)
        & HasProperties(
            key_columns=["id"],
        ),
    )
