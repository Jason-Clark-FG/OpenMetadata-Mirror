...
jest.mock('@openmetadata/ui-core-components', () => ({
  ...
  EmptyPlaceholder: jest
    .fn()
    .mockImplementation(() => <div data-testid="empty-placeholder" />),
}));
...
jest.mock('recharts', () => ({
  ...
  Tooltip: jest.fn().mockImplementation(({ active, content, coordinate }) => (
    <div
...
describe('TestSummaryGraph', () => {
  ...
  it('should activate the tooltip only from the visible status dot', () => {
    ...
    expect(tooltip).toHaveAttribute(ACTIVE_ATTRIBUTE, 'true');
    ...
    fireEvent.mouseEnter(screen.getByTestId('test-summary-custom-tooltip'));
    act(() => {
    ...
    fireEvent.mouseLeave(screen.getByTestId('test-summary-custom-tooltip'));
    act(() => {
      jest.runOnlyPendingTimers();
    });
