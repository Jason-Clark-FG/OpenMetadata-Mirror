...
const ContextCenterMemoriesPage: FC = () => {
  ...
  const [selectedAsset, setSelectedAsset] = useState<MemoryFilterOption>();
  const [selectedAuthor, setSelectedAuthor] = useState<MemoryFilterOption>();
  const [assetOptions, setAssetOptions] = useState<MemoryFilterOption[]>([]);
  ...
  const fetchAssetOptions = useCallback(async (query: string) => {
    ...
    try {
      ...
      const options = response.hits.hits
        .map(({ _source }): MemoryFilterOption | undefined => {
          const source = _source as SearchOptionSource;
          const id = source.id;
          if (!id) {
            return undefined;
          }
          ...
          return {
            ...
            displayName: getOptionLabel(source),
            name: source.name,
      ...
      setIsAssetOptionsLoading(false);
    }
  }, []);
        ...
        <Box align="center" gap={2}>
          <Dropdown.Root
            ...
            <Dropdown.Popover className="tw:w-100">
              ...
              <Dropdown.Menu
                selectedKeys={selectedAsset ? [selectedAsset.id] : []}
                ...
                onAction={(key) => {
                  ...
                  if (next === 'all-assets') {
                    ...
                    const option = assetOptions.find((opt) => opt.id === next);
                    setSelectedAsset(
                      next === selectedAsset?.id ? undefined : option
                    );
                  }
                  if (activeFilter === 'all') {
                    setActiveFilter('');
