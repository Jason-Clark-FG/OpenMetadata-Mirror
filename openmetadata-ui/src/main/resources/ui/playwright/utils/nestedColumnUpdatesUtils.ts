...
interface EntityResponseData {
  fullyQualifiedName: string;
  requestSchema: { schemaFields: NestedField[] };
  messageSchema: { schemaFields: NestedField[] };
  columns: NestedField[];
  fields: NestedField[];
  dataModel: { columns: NestedField[] };
}

export const getNestedColumnDetails = (type: string, data: EntityTypes) => {
  const entityData = data.entityResponseData as unknown as EntityResponseData;
...
export const createWorksheetEntity = async (apiContext: APIRequestContext) => {
  ...
  return {
    ...
    service,
    deleteService: () =>
      worksheetClass.delete(apiContext).then(() => undefined),
    visitPage: async (page: Page) => {
...
export const createFileEntity = async (apiContext: APIRequestContext) => {
  ...
  return {
    entity,
    service,
    deleteService: () => fileClass.delete(apiContext).then(() => undefined),
    visitPage: async (page: Page) => {
