...
public class DataInsightsApp extends AbstractNativeApplication {
  ...
  public void createManifestIndexIfAbsent() {
    DataInsightsSearchInterface searchInterface = getSearchInterface();
    ...
    try {
      if (!searchInterface.manifestIndexExists(manifestName)) {
        ...
        LOG.info("[Data Insights] Created manifest index: {}", manifestName);
      }
    } catch (IOException ex) {
      LOG.error("[Data Insights] Failed to create manifest index: {}", manifestName, ex);
    }
  }
